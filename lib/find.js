...
function contains_path(self, dirname) {
  for (var i=0; i<self.paths.length; i++) {
    ...
    if (path === dirname.substr(0, path.length)) {
      return true;
    }
  }
  return false;
...
// this function corresponds to both match() and sort_matches()
// in ruby code; different loops joined for performance reasons
function match(self, dirname, basename, fn) {
  // function used to calculate "weight" of every basename
  // used for sorting
  function reduce_fn(sum, ext) {
    if (!ext) {
      return sum;
    }
  ...
  for (var i=0; i<entries.length; i++) {
    ...
    var filename = Path.join(dirname, entry);
    ...
    var extnames = entry.replace(basename, '').split('.');
    weights[filename] = extnames.reduce(reduce_fn, 0);
